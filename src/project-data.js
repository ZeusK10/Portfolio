/* eslint-disable import/no-anonymous-default-export */
export default [
  {
    id: 1,
    title: "2.5D Platformer",
    coverImg: "2dplatformer.png",
    img: [
      "2dplatformer_2.png",
      "2dplatformer_3.png",
      "2dplatformer_4.png",
      "2dplatformer_5.png",
      "2dplatformer_6.png",
    ],
    date: "August, 2021",
    gamelink: "https://zeusk10.itch.io/25d-platformer",
    intro:
      "A 2.5D platformer game that focuses on movement with character controller and some game features.",
    desc: "I finished and completed this 2.5D platformer game and created a working moveable player from the scratch using character controller being able to run, jump, roll, etc using some awesome animations. I also converted the concept for ledge grab system, ladder climb, elevator, moving platform to the actual working feature in this game. This took me 1 month to complete this project.",
  },
  {
    id: 2,
    title: "2D Galaxy Space Shooter",
    coverImg: "2d_space_shooter_mainmenu.png",
    img: [
      "space_shooter_2.png",
      "space_shooter_3.png",
      "space_shooter_4.png",
      "space_shooter_5.png",
      "space_shooter_6.png",
      "space_shooter_7.png",
      "space_shooter_8.png",
    ],
    date: "May, 2021",
    gamelink: "https://zeusk10.itch.io/2d-space-shooter",
    intro:
      "A 2D space shooter playable on PC that is similar to classic retro arcade games like Galaga. ",
    desc: "I finished and completed the 2D Game Development Course and developed all major gaming mechanics from scratch from player movement, enemies, spawn managers, powers, speed boosts, powers, effects, menus, UI, and more by using C# and Unity3D.  The project took 4 weeks for me to complete from start to finish.",
  },
  {
    id: 3,
    title: "The Great Fleece",
    coverImg: "the-great-fleece.png",
    date: "June, 2021",
    gamelink: "https://zeusk10.itch.io/the-great-fleece",
    intro:
      "A 3D playable stealth game that focuses on cinematography, stealth and diversion.",
    desc: "I finished and completed this Stealth game and Cinematography course and developed all major gaming mechanics from scratch going from creating cinemachine virtual cameras aligning them to perfectly fit the cutscene and developed some cool looking cutscene using the Timeline feature of unity. The project took 1 week to complete from start to finish.",
  },
  {
    id: 4,
    title: "dungeon escape",
    coverImg: "de.jpg",
    gamelink:
      "https://play.google.com/store/apps/details?id=com.ZeusKGames.DungeonEscape",
    intro:
      "A 2D game that demonstrates an understanding of 2D tile-map implementation, complex class inheritance, UI and shop implementations along with animation implementation and hit box manipulation.",
    desc: "This game also serves as an example on how to implement Google Play advertisements and publishing to the Google Play Store.",
  },
  {
    id: 5,
    title: "Zombie Shooter",
    coverImg: "zs.png",
    intro:
      "A 3D shooter game that focuses on character controller, camera follow, player rotation follow crosshair, raycast, character animation, IK system, etc.",
    desc: "It uses HDRP to create a real like environment and IK system for reload animation.",
  },
  {
    id: 6,
    title: "Guide To Beautiful Game",
    coverImg: "guide.png",
    intro:
      "A 3D platform Game Design that focuses on using assets to create new assets that can be used in the design.",
    desc: "I learned the usage the URP and HDRP in this course along with use of post processing to create attractive game level.Learned about Skyboxes, Lights, altering Texture Maps, Post Processing, Occlusion culling, Particles, Reflection probes, etc for URP and Environmental lighting, fog, lighting effects, baked/realtime lights, lights probes, reflections, volumes, etc in HDRP",
  },
];
